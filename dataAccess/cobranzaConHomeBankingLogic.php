...
function procesarPagoHomeBanking($idCobranza, $idLinkPagos, $matricula, $fechaPago, $importeParcial, $comprobante, $concepto) {
    ...
    if (isset($idLinkPagos)) {
        ...
        $sql="SELECT a.Concepto, a.ImportePrimerVto, a.ImporteSegundoVto
            FROM home_banking_archivo_concepto a
            WHERE a.id = ? AND a.Matricula = ?";
        /*
        ...
        */
        ...
        if (substr($concepto, 0, 1) == '2') {
            $sql="SELECT a.Id, a.ImportePrimerVto, a.ImporteSegundoVto
            FROM home_banking_archivo_concepto a
            INNER JOIN cursosasistente ca ON ca.Id = a.IdAsistente
            WHERE a.Concepto = ? AND a.IdAsistente = ?";
        } else {
            $sql = "SELECT a.Id, a.ImportePrimerVto, a.ImporteSegundoVto
            FROM home_banking_archivo_concepto a
            INNER JOIN colegiado c ON c.Id = a.IdColegiado
            WHERE a.Concepto = ? AND a.Matricula = ?";
        }
        /*
        ...
        */
        ...
        $stmt->bind_result($idHomeBankingArchivoConcepto, $importePrimerVto, $importeSegundoVto);
        $stmt->store_result();
    }
    ...
    if (mysqli_stmt_errno($stmt) == 0) {
        ...
        switch ($concepto) {
            case '001': 
                ...
                */
                $sqlDeuda = "SELECT a.IdDeuda, a.ImportePrimerVto, a.ImporteSegundoVto, cda.Periodo, cdac.Cuota, cdac.Importe, cdac.Recargo
                    FROM home_banking_archivo_concepto_detalle a
                    LEFT JOIN colegiadodeudaanualcuotas cdac ON cdac.Id = a.IdDeuda
                    LEFT JOIN colegiadodeudaanual cda ON cda.Id = cdac.IdColegiadoDeudaAnual
                    WHERE a.IdHomeBankingArchivoConcepto = ?";
                $stmtDeuda = $conect->prepare($sqlDeuda);
                $stmtDeuda->bind_param('i', $idHomeBankingArchivoConcepto);
                $stmtDeuda->execute();
            ...
            case '003': //Cuota PlanPagos
                ...
                $stmtDeuda->bind_param('i', $idLinkPagos);
                */
                $sqlDeuda = "SELECT a.IdDeuda, a.ImportePrimerVto, ppc.IdPlanPagos, ppc.Cuota, ppc.Cuota, ppc.Importe
                    FROM home_banking_archivo_concepto_detalle a
                    INNER JOIN planpagoscuotas ppc ON ppc.Id = a.IdDeuda
                    WHERE a.IdHomeBankingArchivoConcepto = ?";
                $stmtDeuda = $conect->prepare($sqlDeuda);
                $stmtDeuda->bind_param('i', $idHomeBankingArchivoConcepto);
                $stmtDeuda->execute();
            ...
            case ($concepto >= '004' && $concepto <= '013'):
                ...
                $stmtDeuda->bind_param('i', $idLinkPagos);
                */
                $sqlDeuda = "SELECT a.IdDeuda, a.ImportePrimerVto, cda.Periodo, cdac.Cuota, cdac.Importe
                    FROM home_banking_archivo_concepto_detalle a
                    INNER JOIN colegiadodeudaanualcuotas cdac ON cdac.Id = a.IdDeuda
                    INNER JOIN colegiadodeudaanual cda ON cda.Id = cdac.IdColegiadoDeudaAnual
                    WHERE lpd.IdLinkPagos = ?";
                $stmtDeuda = $conect->prepare($sqlDeuda);
                $stmtDeuda->bind_param('i', $idHomeBankingArchivoConcepto);
            ...
            case '014':
                ...
                $stmtDeuda->bind_param('i', $idLinkPagos);
                */
                $sqlDeuda = "SELECT a.IdDeuda, a.ImportePrimerVto, cda.Periodo
                    FROM home_banking_archivo_concepto_detalle a
                    INNER JOIN colegiadodeudaanualtotal cdat ON cdat.Id = a.IdDeuda
                    INNER JOIN colegiadodeudaanual cda ON cda.Id = cdat.IdColegiadoDeudaAnual
                    WHERE lpd.IdLinkPagos = ?";
                $stmtDeuda = $conect->prepare($sqlDeuda);
                $stmtDeuda->bind_param('i', $idHomeBankingArchivoConcepto);
            ...
            case ($concepto >= '200' && $concepto <= '299'): //Cuota Curso
                ...
                $stmtDeuda->bind_param('i', $idLinkPagos);
                */
                $sqlDeuda = "SELECT a.IdDeuda, a.ImportePrimerVto, cac.Cuota, cac.Importe
                    FROM home_banking_archivo_concepto_detalle a
                    INNER JOIN cursosasistentecuotas cac ON cac.Id = a.IdDeuda
                    WHERE lpd.IdLinkPagos = ?";
                $stmtDeuda = $conect->prepare($sqlDeuda);
                $stmtDeuda->bind_param('i', $idHomeBankingArchivoConcepto);
                $stmtDeuda->execute();
